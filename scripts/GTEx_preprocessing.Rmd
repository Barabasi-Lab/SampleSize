...
## Change format of expression to long format

We will change the format of the gene expression dataframe to long format, as it is easier to process the data in rows than in columns in R:

```{r change_to_long_format}
#tpm_long_df = tpm_df %>% pivot_longer(!c(Name, Description), names_to = "Sample", values_to = "TPM")
#dim(tpm_long_df)
#head(tpm_long_df)
...
for (tissue in unique(samples_df$SMTSD.no.sp.char)){
  for (sex in rownames(sex_to_id)){
    ...
    if (length(samples_tissue_sex) > 10){
      ...
      tpm_tissue_sex_filt_df = tpm_tissue_sex_df[tpm_tissue_sex_df$Description %in% info_filt_df$Description,]
