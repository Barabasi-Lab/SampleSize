...
We read the PPI network, get the LCC, order them, map the gene IDs to symbols:
...
print(length(lcc_nodes))

# Filter interactions not in LCC
PPI_symbol = PPI_symbol %>% filter((proteinA_symbol %in% lcc_nodes) & (proteinB_symbol %in% lcc_nodes))

# Order PPI interactions alphabetically
PPI_ordered = data.frame(matrix(ncol=4,nrow=0, dimnames=list(NULL, c("proteinA", "proteinB", "proteinA_symbol", "proteinB_symbol"))))
for(row_id in row.names(PPI_symbol)){
  ppi_row = PPI_symbol %>% slice(as.numeric(row_id))
  ppi_row_sort = ppi_row %>% select(proteinA_symbol, proteinB_symbol) %>% sort()
  if(names(ppi_row_sort)[1] == "proteinB_symbol"){
    ppi_row_ordered = data.frame(proteinA=ppi_row[[2]], proteinB=ppi_row[[1]], proteinA_symbol=ppi_row[[4]], proteinB_symbol=ppi_row[[3]])
    PPI_ordered = rbind(PPI_ordered, ppi_row_ordered)
  } else {
    PPI_ordered = rbind(PPI_ordered, ppi_row)
  }
}
