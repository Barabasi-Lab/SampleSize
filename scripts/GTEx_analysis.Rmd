...
```{r define_files}
# Define input files
subjects_file = '/home/j.aguirreplans/Databases/GTEx/v8/GTEx_Analysis_v8_Annotations_SubjectPhenotypesDS.txt'
samples_file = '/home/j.aguirreplans/Databases/GTEx/v8/GTEx_Analysis_v8_Annotations_SampleAttributesDS.txt'
tpm_file = '/home/j.aguirreplans/Databases/GTEx/v8/GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_tpm.gct'
tpm_median_file = '/home/j.aguirreplans/Databases/GTEx/v8/GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_median_tpm.gct'
...
samples_subtissues_plot <- smtsd_samples_count %>%
  ...
  ggplot( aes(x=SMTSD, y=n)) +
    ...
    xlab("") +
    theme_bw()
ggsave('/home/j.aguirreplans/Projects/Scipher/SampleSize/data/out/GTEx_analysis/num_samples_vs_subtissues.png', plot=samples_subtissues_plot, type='cairo', dpi = 300)
...
samples_subtissues_by_sex_plot <- smtsd_samples_count_by_sex %>%
  ...
  ggplot( aes(x=SMTSD, y=n, fill=sex)) +
    ...
    xlab("") +
    theme_bw()
ggsave('/home/j.aguirreplans/Projects/Scipher/SampleSize/data/out/GTEx_analysis/num_samples_vs_subtissues_by_sex.png', plot=samples_subtissues_by_sex_plot, type='cairo', dpi = 300)
...
### Specific for whole blood

Let's analyze the distribution of TPMs specifically for Whole Blood.

```{r}
tissue <- "Whole Blood"
samples_tissue_df <- samples_df[(samples_df$SMTSD == tissue) & (samples_df$SAMPID %in% colnames(tpm_df)) ,]
samples_tissue <- unique(samples_tissue_df$SAMPID)
subjects_tissue <- unique(samples_tissue_df$SUBJID)
tpm_tissue_df <- tpm_df %>% select(c("Name", "Description", samples_tissue))
...
```{r}
info_tissue_filt_df = info_tissue_df[(info_tissue_df$sum > 0) & (info_tissue_df$mean > 0) & (info_tissue_df$median > 0) & (info_tissue_df$sd > 0) & (info_tissue_df$zero_sum_counts < dim(tpm_tissue_df[, -1])[2]/2) ,]
