...
def main():
    ...
    python /home/j.aguirreplans/Projects/Scipher/SampleSize/scripts/calculate_kendall_cluster.py -n /scratch/j.aguirreplans/Scipher/SampleSize/networks_nonresponders_wto_G10000_N20 -o /home/j.aguirreplans/Projects/Scipher/SampleSize/data/out/analysis_sample_size_G10000_20
    """
    options = parse_options()
    calculate_kendall_cluster(options)
...
def parse_options():
    ...
    parser = optparse.OptionParser("calculate_kendall_cluster.py  -s SIZE")
...
def calculate_kendall_cluster(options):
    """
    ...
    script_file = os.path.join(src_path, 'calculate_kendall.py')
    ...
    for size in size_list:
        ...
        bash_script_name = 'kendall_size_{}.sh'.format(size)
        bash_script_file = os.path.join(dummy_dir, bash_script_name)

        output_file = os.path.join(output_dir, 'kendall_size_{}.txt'.format(size))
        #if not fileExist(bash_script_file):
        if not fileExist(output_file):

            command = 'python {} -n {} -o {} -s {} -r {}'.format(script_file, networks_dir, output_dir, size, repetitions)
            print(command)
