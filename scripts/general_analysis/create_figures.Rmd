...
scaling_relation_plot_lessdatasets = scaling_relation_lessdatasets_df %>%
  ...
  geom_abline(aes(intercept=intercept, slope=slope, col=r2labels), size=1) +
  theme_linedraw() +
  xlab("log(n)") +
  ylab(bquote(bold(log((S[n]-S[n-1])/S[n-1])))) +
...
a_vs_fraction_sig_correlations_lessdatasets_plot <- sample_size_vs_a_plot_lessdatasets_df %>%
    ...
    geom_abline(aes(slope = slope_avsfr,
                    ...
                    col = lm_cor_result_avsfr),
              linetype = "dashed", 
              show.legend = FALSE) +
  scale_color_manual(values = c("gray50")) +
  annotate("text", x = 2.00, y = 0.5, label = lm_cor_result_avsfr, size = 5) +
  xlab(expression(alpha)) +
  ylab("Frac. correlations above 0.2") +
  theme_linedraw() +
  theme(aspect.ratio = 1, 
...
(((module_rlcc_by_correlation_plots[[1]] | module_signif_by_correlation_plots[[1]]) + 
  ...
  plot_annotation(tag_levels = 'A') & # Include letters 
    theme(element_text(face = 'bold', size = 17), legend.text = element_text(size = 11)) # that are bold
