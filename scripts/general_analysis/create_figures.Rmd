...
scaling_relation_comparison_df <- scaling_relation_comparison_df %>%
  dplyr::mutate(
    r2labels = dplyr::case_when(
      model == "Power law" ~ paste("Power law: ", unique((scaling_relation_comparison_df %>% filter(model == "Power law"))$adj.r.squared), sep = ""),
      model == "Exponential decay" ~ paste("Exp. decay: ", unique((scaling_relation_comparison_df %>% filter(model == "Exponential decay"))$adj.r.squared), sep = ""),
      model == "Constant discovery" ~ paste("Constant: ", unique((scaling_relation_comparison_df %>% filter(model == "Constant discovery"))$adj.r.squared), sep = ""),
      TRUE ~ ""
    )
  )
