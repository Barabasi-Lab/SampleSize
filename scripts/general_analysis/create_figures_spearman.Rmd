...
a_vs_fraction_sig_correlations_lessdatasets_plot <- sample_size_vs_a_plot_lessdatasets_df %>%
    ...
    geom_abline(aes(slope = slope_avsfr,
                ...
                col = "gray50",
                show.legend = FALSE) +
  annotate("text", x = 2, y = 0.6, label = lm_cor_result_avsfr, size = 5) +
  xlab(expression(alpha)) +
  ylab("Frac. correlations above 0.2") +
  theme_linedraw() +
  theme(aspect.ratio = 1, 
...
for (x in 1:length(parameters)){
  ...
  distribution_plots[[parameter]] = distribution_plot
...
```{r}
a_vs_fraction_corr_file <- paste(
  tables_dir, 
  "/a_vs_fraction_sig_correlations_",
  type_data_selection,
  ".txt", 
  sep = ''
)
...
a_vs_ss_lin_log_plot <- sd_genes_vs_ss_df %>%
    ...
    scale_color_manual(values = c("gray50")) +
    #annotate("text", x = 4500, y = 2.05, label = lm_cor_result_ssvsa, size = 5) +
    annotate("text", x = 2.75, y = 2.1, label = lm_cor_result_ssvsa, size = 5) +
    xlab("log10(n) to get 50% sig. links") +
    ylab(expression(bold(alpha))) +
    theme_linedraw() +
    theme(aspect.ratio = 1, 
...
cv_vs_ss_plot <- sd_genes_vs_ss_df %>%
    ...
    geom_abline(aes(slope = slope_cvvsss,
              ...
              show.legend = FALSE) +
    scale_color_manual(values = c("gray50")) +
    annotate("text", x = 1100, y = 0.45, label = lm_cor_result_cvvsss, size = 5) +
...
ranking_nodes_df= data.table::fread(ranking_nodes_file)
...
coexpression_pairs_filt = data.table::fread(coexpression_pairs_file)
