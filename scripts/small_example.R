...
results_selected_df = results_df %>% filter((type_dataset %in% c("gtex:whole.blood")) & (method %in% c("pearson")) & (type_correlation %in% c("all")) & (threshold %in% c(0.05)) & (size <= 100)) # For a test
...
topology_results_file = paste(output_dir, 'topology_results_pearson_pval_0.05_small_example.txt', sep='/')
results_selected_df %>% fwrite(topology_results_file)
topology_results_by_size_file = paste(output_dir, 'topology_results_mean_pearson_pval_0.05_small_example.txt', sep='/')
topology_results_selected_by_size_df %>% fwrite(topology_results_by_size_file)
predictions_file = paste(output_dir, 'predictions_pearson_pval_0.05_small_example.txt', sep='/')
predicted_results_df %>% fwrite(predictions_file)
analytical_results_file = paste(output_dir, 'analytical_model_results_pearson_pval_0.05_small_example.txt', sep='/')
topology_results_selected_analytical_df %>% fwrite(analytical_results_file)
analytical_summary_file = paste(output_dir, 'analytical_model_summary_pearson_pval_0.05_small_example.txt', sep='/')
analytical_model_summary_df %>% fwrite(analytical_summary_file)
analytical_regression_results_file = paste(output_dir, 'analytical_model_regression_results_pearson_pval_0.05_small_example.txt', sep='/')
...
output_dir = '/home/j.aguirreplans/Projects/Scipher/SampleSize/scripts/SampleSizeShiny/data/example_pearson_pval_0.05'
topology_results_file = paste(output_dir, '/', 'topology_results_norm_', topology_type_normalization, '_pearson_pval_0.05_small_example.txt', sep='')
results_selected_norm_df %>% fwrite(topology_results_file)
topology_results_by_size_file = paste(output_dir, '/', 'topology_results_mean_norm_', topology_type_normalization, '_pearson_pval_0.05_small_example.txt', sep='')
topology_results_selected_by_size_norm_df %>% fwrite(topology_results_by_size_file)
predictions_file = paste(output_dir, '/', 'predictions_', topology_type_normalization, '_pearson_pval_0.05_small_example.txt', sep='')
predicted_results_norm_df %>% fwrite(predictions_file)
analytical_results_file = paste(output_dir, '/', 'analytical_model_results_', topology_type_normalization, '_pearson_pval_0.05_small_example.txt', sep='')
