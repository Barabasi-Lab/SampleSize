...
for (tissue in tissues){
  for (sex in rownames(sex_to_id)){
    ...
    if (length(tissue_sex_samples) > 10){
      ...
      output_file = paste(sampling_dir, paste(paste('RNAseq_samples', tissue, sex, sep="_"), '.txt', sep=''), sep='/')
      subsamples_df %>% write.table(output_file, quote = F, sep='\t', row.names = F, col.names = T)
    }
  }
}
