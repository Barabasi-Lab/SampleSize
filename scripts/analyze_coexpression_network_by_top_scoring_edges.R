...
read_coexpression_network <- function(coexpression_network_file){
  ...
  if(grepl('wto', coexpression_network_file, fixed = TRUE)){
    ...
    coexpression_network_df = coexpression_network_df %>% filter(pvalue < 0.05)
...
edge_number_list = c(100000, 300000, 500000, 1000000)
for(edge_number_threshold in edge_number_list){
