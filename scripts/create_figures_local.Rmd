...
```{r}
input_dir = '/Users/j.aguirreplans/Desktop/SampleSizeShiny/data'
plots_dir = '/Users/j.aguirreplans/Desktop/plots'
tables_dir = '/Users/j.aguirreplans/Desktop/tables'
...
input_dir = '/Users/j.aguirreplans/Desktop/SampleSizeShiny/data/example_pearson_pval_0.05'
...
```{r}
datasets_selected = c("gtex:breast.mammary.tissue", "gtex:whole.blood", "tcga:tcga-brca_female", "scipher:scipher.sample.per.patient.baseline")
results_curve_df = results_selected_norm_df %>%
  dplyr::filter(type_dataset %in% datasets_selected) %>%
  dplyr::select(size, rep, unnorm, type_dataset) %>%
  mutate(type_dataset = replace(type_dataset, type_dataset == "scipher:scipher.sample.per.patient.baseline", "R. arthritis")) %>%
...
plot_file = paste(plots_dir, "curve_pearson_pval_0.05_scipher.png", sep="/")
ggsave(
  plot_file,
  plot=curve_plot,
  dpi = 1200,
...
plot_file = paste(plots_dir, "scaling_relation_pearson_pval_0.05_scipher.png", sep="/")
ggsave(
  plot_file,
...
plot_file = paste(plots_dir, "regression_pearson_pval_0.05_scipher.png", sep="/")
ggsave(
  plot_file,
  plot=regression_plot,
  dpi = 1200,
...
```{r}
datasets_selected = c("GTEx: Breast", "GTEx: Whole blood", "TCGA: Breast cancer", "R. arthritis")
...
plot_file = paste(plots_dir, "prediction_pearson_pval_0.05_scipher.png", sep="/")
print(prediction_plot)
ggsave(
  plot_file,
  plot=prediction_plot,
...
((curve_plot + labs(tag = 'A')) +
  ...
  (prediction_plot + labs(tag = 'E')) +
  (a_vs_fraction_sig_correlations_plot + labs(tag = 'F')) +
  (distribution_plots[[3]] + labs(tag = 'G')) +
...
all_topology_results_file = '/Users/j.aguirreplans/Desktop/SampleSizeShiny/data/analysis_topology.csv'
...
```{r}
input_data_file = "/Users/j.aguirreplans/Desktop/tables/analysis_specific_edges_pearson_gtex_Whole.Blood.txt"
...
```{r}
sd_table_file = paste(tables_dir, "analysis_sd_coexpression_weight.txt", sep="/")
...
sd_plot_file = paste(plots_dir, "analysis_sd_coexpression_weight.png", sep="/")
sd_plot = sd_table_df %>%
...
```{r}
all_disease_genes_results_file = '/Users/j.aguirreplans/Desktop/SampleSizeShiny/data/analysis_disease_genes.csv'
...
threshold_selected = 0.05
coexpression_pairs_file = sprintf("/Users/j.aguirreplans/Desktop/tables/ppi_analysis_pairs_pval_%s_%s_%s.txt", as.character(threshold_selected), dataset, type_dataset)
coexpression_pairs_filt = fread(coexpression_pairs_file)
perf_file = sprintf("/Users/j.aguirreplans/Desktop/tables/ppi_coexpression_analysis_ppi_vs_random_far_pairs_performance_pval_%s_%s_%s.txt", as.character(threshold_selected), dataset, type_dataset)
...
```{r}
violinplot_ppi_coexpression_by_distances_file = sprintf("/Users/j.aguirreplans/Desktop/plots/ppi_coexpression_violinplot_by_ppi_distance_pval_%s_%s_%s.png", as.character(threshold_selected), dataset, type_dataset)
...
```{r}
violinplot_ppi_coexpression_by_types_file =  sprintf("/Users/j.aguirreplans/Desktop/plots/ppi_coexpression_violinplot_by_pair_type_pval_%s_%s_%s.png", as.character(threshold_selected), dataset, type_dataset)
...
auc_plot_file =  sprintf("/Users/j.aguirreplans/Desktop/plots/ppi_coexpression_perf_AUC_pval_%s_%s_%s.png", as.character(threshold_selected), dataset, type_dataset)
ggsave(
  auc_plot_file,
  plot = auc_plot,
  dpi = 1200,
...
```{r}
# Use patchwork to concatenate plots
((violinplot_ppi_coexpression_by_distances + theme(legend.position="bottom")) |
  (violinplot_ppi_coexpression_by_types + theme(legend.position="bottom")) |
  auc_plot) + 
  plot_annotation(tag_levels = 'A') & # Include letters 
    theme(plot.tag = element_text(face = 'bold')) # that are bold
