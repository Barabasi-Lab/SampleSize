...
Calculate the distance matrices if necessary:
...
Filter the matrix of distances between protein/gene pairs:
...
Select random control interactions of the same size as the PPI:
...
Select random control interactions of the same size as the NCI that are far in the NCI:
...
Plot the co-expression of pairs of proteins/genes grouped by different categories:
...
#colors_defined_filt = colors_defined[names(colors_defined) %in% c("PPI", "PPI & iNCI", "Random pairs in PPI", "Random pairs in PPI (dist. > 4)", "Random pairs in PPI & iNCI (dist. > 4)")]
colors_defined_filt = colors_defined[names(colors_defined) %in% c("PPI", "Random", "Random (distance > 4)")]
boxplot_ppi_coexpression_by_types = coexpression_pairs_filt %>%
  ...
  mutate(category = replace(category, category == "random_far_not_ppi", "Random (distance > 4)")) %>%
  #mutate(category = replace(category, category == "random_far_not_nci", "Random pairs in PPI+iNCI (d > 4)")) %>%
  #mutate(category = factor(category, levels = c("PPI","PPI & iNCI", "Random pairs in PPI","Random pairs in PPI (dist. > 4)","Random pairs in PPI & iNCI (dist. > 4)"))) %>%
  mutate(category = factor(category, levels = c("PPI", "Random", "Random (distance > 4)"))) %>%
  ggplot() +
  aes(x = reorder(size, as.numeric(size)),
      y = abs(correlation),
      fill = category,
...
dunn_results_across_types_by_sizes_df = data.frame(matrix(ncol=length(c("size", "Comparison", "Z", "P.unadj", "P.adj")),nrow=0, dimnames=list(NULL, c("size", "Comparison", "Z", "P.unadj", "P.adj"))))
for(size_selected in sort(unique(ppi_coexpression_across_types_by_sizes_df$size))){
  # Filter co-expression by size
  ppi_coexpression_by_selected_size_df = ppi_coexpression_across_types_by_sizes_df %>% filter(size == size_selected)
  # Run KW test across types
  kw_across_types = ppi_coexpression_by_selected_size_df %>% 
    mutate(category = factor(category, levels = c("ppi", "random_not_ppi", "random_far_not_ppi"))) %>% 
  ...
  dunn_across_types = ppi_coexpression_by_selected_size_df %>% 
    mutate(category = factor(category, levels = c("ppi", "random_not_ppi", "random_far_not_ppi"))) %>% 
    FSA::dunnTest(abs(correlation) ~ category, .)
  # Save result
  dunn_results_across_types_by_sizes_df= rbind(dunn_results_across_types_by_sizes_df, cbind(data.frame(size=size_selected), dunn_across_types$res))
  rm(ppi_coexpression_by_selected_size_df)
}
rm(ppi_coexpression_across_types_by_sizes_df)

# Merge and save results
test_results_across_types_by_sizes_df = kw_results_across_types_by_sizes_df %>% full_join(dunn_results_across_types_by_sizes_df, by=c("size")) %>% rename("comparison"="Comparison", "dunn.z"="Z", "dunn.p.unadj"="P.unadj", "dunn.p.adj"="P.adj")
...
```{r}
head(test_results_across_types_by_sizes_df)
```

```{r}
test_results_across_types_by_sizes_df %>% 
  select(size, comparison, dunn.z, dunn.p.adj) %>% 
  filter(comparison %in% c("ppi - random_far_not_ppi")) %>% 
  unique() %>% 
  arrange(comparison, size)
```
...
perf_df = data.frame(matrix(ncol=length(c("size", "x", "y", "type", "base", "AUC", "AUCPR")),nrow=0, dimnames=list(NULL, c("size", "x", "y", "type", "base", "AUC", "AUCPR"))))
