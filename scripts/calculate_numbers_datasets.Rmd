...
scipher_rnaseq_dir = "/home/j.aguirreplans/Projects/Scipher/SampleSize/data/Dec2021/00_data/rnaseq_filtered_files_by_group"
...
rnaseq_files = list.files(tcga_dir)
for (type_counts in c("reads", "tpm")){
  for (type_tcga_tissue in c("tumor", "normal")){
    ...
    for (rnaseq_file in rnaseq_files){
      ...
      if (nrow(tcga_meta_df) == 0){
        tcga_meta_df = data.frame(dataset=rnaseq_spl[1], type_dataset=rnaseq_spl[3], type_counts=type_counts, type_tcga_tissue=type_tcga_tissue, sex=sex, num_samples=ncol(rnaseq[,-1]), num_genes=nrow(rnaseq[,-1]))
      } else {
        tcga_meta_df = rbind(tcga_meta_df, data.frame(dataset=rnaseq_spl[1], type_dataset=rnaseq_spl[3], type_counts=type_counts, type_tcga_tissue=type_tcga_tissue, sex=sex, num_samples=ncol(rnaseq[,-1]), num_genes=nrow(rnaseq[,-1])))
      }
    }
  }
}
...
```{r}
gtex_meta_df$type_tcga_tissue = NA
scipher_meta_df$type_tcga_tissue = NA
num_df = rbind(gtex_meta_df, tcga_meta_df, scipher_meta_df)
head(num_df)
```
...
pairs_df = data.frame(gtex_tissue=pairs1, tcga_tissue=pairs2) %>% arrange(gtex_tissue) %>% 
  inner_join((gtex_meta_df %>% select(-dataset, -type_tcga_tissue) %>% rename(c("num_samples_gtex"="num_samples", "num_genes_gtex"="num_genes"))), by=c("gtex_tissue"="type_dataset")) %>% 
...
pairs_df = pairs_df %>% arrange(desc(sum_num_samples)) %>% relocate(type_tcga_tissue, .after = sex) %>% relocate(num_genes_gtex, .after = type_tcga_tissue) %>% relocate(num_genes_tcga, .after = num_genes_gtex) %>% relocate(sum_num_genes, .after = num_genes_tcga)
head(pairs_df, 10)
```
