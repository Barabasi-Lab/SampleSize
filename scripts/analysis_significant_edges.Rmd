...
topology_df %<>% filter((dataset == "scipher") & (type_dataset == "complete_dataset") & (method == "pearson") & (threshold == 0.05) & is.na(disparity))
head(topology_df)
...
lm_res = summary(lm(num_edges~log(size), data=topology_df))
print(lm_res)
formula_name = paste("F(x) = ln(x)*", round(coef(lm_res)[2]),  round(coef(lm_res)[1]), sep="")
...
ggplot(topology_df, aes(x=size, y=num_edges)) + 
    ...
    geom_line(data=topology_df, aes(x=size, y=(log(size)*coef(lm_res)[2] + coef(lm_res)[1]), color=formula_name), size=1) +
...
prediction_num_edges_df = data.frame(size=num_samples, num_edges=(log(num_samples)*coef(lm_res)[2] + coef(lm_res)[1]))
...
plot_num_edges_num_samples_file = paste(plots_dir, "scipher_spearman_pval_0.05_prediction_num_edges_vs_num_samples.png", sep="/")
ggsave(
  plot_num_edges_num_samples_file,
  dpi = 1200,
  width = 15000,
...
```{r}
ggplot(topology_df, aes(x=size, y=log(num_edges))) + 
    geom_point(size=3, col=2, fill=2) +
    #geom_smooth(method = "lm", formula = y~log10(x), col="red", fill="#69b3a2", se=TRUE) +
    #ggtitle("Ln num. of significant edges vs. Num. of samples") +
...
```{r}
ggplot(topology_df, aes(x=log(size), y=num_edges)) + 
    geom_point(size=3, col=2, fill=2) +
    #geom_smooth(method = "lm", formula = y~log(x), col="red", fill="#69b3a2", se=TRUE) +
    #ggtitle("Num. of significant edges vs. Ln num. of samples") +
...
```{r}
ggplot(topology_df, aes(x=log(size), y=log(num_edges))) + 
    geom_point(size=3, col=2, fill=2) +
    #geom_smooth(method = "lm", formula = y~log(x), col="red", fill="#69b3a2", se=TRUE)
    #ggtitle("Ln num. of significant edges vs. Ln num. of samples") +
...
ggplot(topology_df, aes(x=size, y=num_edges)) + 
    ...
    geom_line(data=topology_by_size_df, aes(x=size, y=mean, colour="Mean"), size=1) +
...
### Plot number of samples vs. significant edges for GTEx Whole Blood
...
topology_df %<>% filter(!(size == "all"))
topology_df$size = as.numeric(topology_df$size)
topology_df %<>% filter(((dataset == "gtex") | (dataset == "scipher")) & ((type_dataset == "Whole.Blood") | (type_dataset == "complete_dataset")) & (method == "pearson") & (threshold == 0.05) & is.na(disparity))
head(topology_df)
...
```{r}
lm_res_scipher = summary(lm(num_edges~log(size), data=(topology_df %>% filter(dataset=="scipher"))))
formula_name_scipher = paste("F(x) = ln(x)*", round(coef(lm_res_scipher)[2]),  round(coef(lm_res_scipher)[1]), sep="")
lm_res_gtex = summary(lm(num_edges~log(size), data=(topology_df %>% filter(dataset=="gtex"))))
formula_name_gtex = paste("F(x) = ln(x)*", round(coef(lm_res_gtex)[2]),  round(coef(lm_res_gtex)[1]), sep="")

ggplot(topology_df, aes(x=size, y=num_edges, col=dataset)) + 
    geom_point(alpha=0.5, size=3) +
    geom_line(data=topology_by_size_df, aes(x=size, y=mean, col=dataset), size=1) +
    geom_line(data=topology_df, aes(x=size, y=(log(size)*coef(lm_res_scipher)[2] + coef(lm_res_scipher)[1]), color=formula_name_scipher), size=1) +
    geom_line(data=topology_df, aes(x=size, y=(log(size)*coef(lm_res_gtex)[2] + coef(lm_res_gtex)[1]), color=formula_name_gtex), size=1) +
    ggtitle("Num. of significant edges vs. Num. of samples") +
    labs(x="Number of samples", y = "Number of significant edges") +
    theme_linedraw() +
    theme(plot.title =  element_text(size = 17, face="bold"), axis.title = element_text(size = 16, face="bold"), axis.text = element_text(size = 15), legend.text = element_text(size = 14), legend.position="bottom") +
...
plot_num_edges_num_samples_file = paste(plots_dir, "gtex_Whole.Blood_spearman_pval_0.05_num_edges_vs_num_samples.png", sep="/")
ggsave(
  plot_num_edges_num_samples_file,
  dpi = 1200,
  width = 15000,
...
plot_num_edges_num_samples_file = paste(plots_dir, "gtex_Whole.Blood_spearman_pval_0.05_prediction_num_edges_vs_num_samples.png", sep="/")
ggsave(
  plot_num_edges_num_samples_file,
  dpi = 1200,
  width = 15000,
