...
# For GSE193677-Rectum_inflamed vs. GSE193677-Rectum_control
input_dir = "/scratch/j.aguirreplans/Scipher/SampleSize/differential_coexpression_analysis/reads/GSE193677-Rectum_inflamed___GSE193677-Rectum_control/consensus"
plots_dir = "/home/j.aguirreplans/Projects/Scipher/SampleSize/data/out/plots/plots_differential_coexpression/GSE193677-Rectum_inflamed___GSE193677-Rectum_control"
tables_dir = "/home/j.aguirreplans/Projects/Scipher/SampleSize/data/out/tables/tables_differential_coexpression/GSE193677-Rectum_inflamed___GSE193677-Rectum_control"
name_D = "inflamed"
name_N = "control"
disease_name_in_associations_file = "inflammatory.bowel.diseases"
pval_correction_field = "pval_Phi_Tilde.adj.bonf"
pval_threshold = 0.00001
nodes_to_follow = c("NOD2", "NOD1", "TLR1", "TLR3", "TLR4", "TLR6", "HLA-DRB1")
...
plot_flow = nodes_filtered_col_df %>% 
  ...
  scale_fill_manual(values=setNames(nodes_filtered_col_df$rgb_col, nodes_filtered_col_df$Phi_name)) +
  scale_color_manual(values=setNames(nodes_filtered_col_df$rgb_col, nodes_filtered_col_df$Phi_name)) +
  ...
  if(!(length(reps) == 1 && reps == "consensus-consensus")){
    for (important_node in nodes_to_follow){
      ...
      important_node_plot = important_df %>% 
        ...
        scale_fill_manual(values=setNames(important_df$rgb_col, important_df$Phi_name)) +
        guides(fill=guide_legend(title="Gene category")) +
        theme_linedraw() +
        theme(aspect.ratio=1,
              plot.title =  element_text(size = 15, face="bold"), 
    ...
    for (important_node in nodes_to_follow){
      ...
      important_node_plot = important_df %>% 
        ...
        scale_color_manual(values=setNames(important_df$rgb_col, important_df$Phi_name)) +
        theme_linedraw() +
        theme(aspect.ratio=1,
              plot.title =  element_text(size = 15, face="bold"), 
              axis.title = element_text(size = 14, face="bold"), 
...
# Create last column of plot (e.g., size 100)
change_of_category_size100_df = change_of_category_df %>%
  filter(size.curr == tail(sizes_to_plot, n=1)) %>%
  ...
  mutate(transition_name_simple = as.character(Phi_name.prev)) %>%
...
plot_flow = change_of_category_col_df %>% 
  ...
  geom_flow(stat = "alluvium", lode.guidance = "frontback") +
  geom_stratum() +
  scale_fill_manual(values=setNames(change_of_category_col_df$rgb_col, change_of_category_col_df$transition_name_simple)) + # If I want to plot only specific elements in the legend, use the argument breaks with a list of the elements that I want to show
  scale_color_manual(values=setNames(change_of_category_col_df$rgb_col, change_of_category_col_df$transition_name_simple)) +
  xlab("Number of samples") +
  ylab("Number of disease genes") +
  guides(fill=guide_legend(title="Gene category")) +
  theme_linedraw() +
