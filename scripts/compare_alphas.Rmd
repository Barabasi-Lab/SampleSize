...
```{r}
list_alphas = c(round(analytical_model_selected$a, 2), 1.25, 1.5, 1.75, 2)
...
comparison_alphas_plot = predicted_results_df %>%
  ggplot(aes(x=size, y=norm, col=a)) +
  geom_line(size=1) +
  scale_x_continuous(trans = scales::log10_trans()) +
  scale_color_manual(name = "Prob. reveal link", values = as.vector(name2color[levels(factor(predicted_results_df$a))])) +
  theme_linedraw() +
