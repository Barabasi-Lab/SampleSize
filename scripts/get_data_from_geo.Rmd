...
```{r}
gse <- getGEO(geo_id, GSEMatrix = TRUE, destdir=geo_id_dir)
show(gse)
```
...
metadata_complete_file = paste(geo_id_dir, "/metadata_complete.txt", sep="")
...
```{r}
filePaths = getGEOSuppFiles(geo_id, makeDirectory = FALSE, baseDir = geo_id_dir)
filePaths
```
...
metadata_simple_file = paste(geo_id_dir, "/metadata_simple.txt", sep="")
...
```{r}
raw_counts_original_file = paste(geo_id_dir, "/GSE193677_MSCCR_Biopsy_counts.txt.gz", sep="")
```
...
```{r}
raw_counts_processed_file = paste(geo_id_dir, "/GSE193677_MSCCR_Biopsy_counts_processed.txt", sep="")
