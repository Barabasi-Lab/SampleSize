...
We observe that some of the biopsies from the same subject can be both inflamed or non-inflamed, such as subjects 4 and 5:

```{r}
metadata_df %>% dplyr::select(subject_id, inflammation) %>% group_by(subject_id, inflammation) %>% summarize(n_inflammation=n()) %>% head(10)
```

Number of subjects from a specific disease group:
