...
reads_file = paste(gtex_dir, 'GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_reads.gct', sep='/')
...
```{r}
if(type_counts == "tpm"){
  rnaseq = fread(tpm_file)
} else if(type_counts == "reads"){
  rnaseq = fread(reads_file)
} else {
...
```{r}
rnaseq_no_duplicates_file = paste(gtex_dir, paste('GTEx_RNASeq_gene_', type_counts, '_no_duplicates.gct', sep=""), sep='/')
...
```{r}
rnaseq_no_filt_dir = paste(gtex_dir, paste(type_counts, '_by_tissue', sep=''), sep='/')
rnaseq_filt_dir = paste(gtex_dir, paste(type_counts, '_filtered_files_by_tissue', sep=''), sep='/')
...
for (tissue in unique(samples_df$SMTSD.no.sp.char)){
  for (sex in rownames(sex_to_id)){
    ...
    if (sex == ""){
      ...
      rnaseq_tissue_sex_no_filt_file = paste(rnaseq_filt_dir, paste("gtex_rnaseq_", tissue, ".gct", sep=''), sep='/')
      ...
      rnaseq_tissue_sex_no_filt_file = paste(rnaseq_filt_dir, paste("gtex_rnaseq_", tissue, "_", sex, ".gct", sep=''), sep='/')
    ...
    if (length(samples_tissue_sex) > 10){
      ...
      info_df$zero_sum_counts = as.numeric(apply(rnaseq_tissue_sex_df[,-1], 1, function(i) sum(i == 0) )) # Number of times that the counts are equal to 0
      # Filter low expressed genes
      if(type_counts == "tpm"){
        info_filt_df = info_df[(info_df$sum > 0) & (info_df$mean > 1) & (info_df$median > 1) & (info_df$sd > 0) & (info_df$zero_sum_counts < dim(rnaseq_tissue_sex_df[, -1])[2]/2) ,]
      } else if(type_counts == "reads"){
        info_filt_df = info_df[(info_df$sum > 0) & (info_df$mean > 5) & (info_df$median > 5) & (info_df$sd > 1) & (info_df$zero_sum_counts < dim(rnaseq_tissue_sex_df[, -1])[2]/2) ,]
      } else {
        stop("Unknown type of data")
      }
