...
topology_df = data.frame(matrix(ncol=16,nrow=0, dimnames=list(NULL, c("type_correlation", "threshold", "disparity", "num_nodes", "num_edges", "av_degree", "av_path_length", "av_clustering_coef", "num_components", "num_lcc_nodes", "num_lcc_edges", "lcc_z", "lcc_pvalue", "max_k", "num_main_core_nodes", "num_main_core_edges"))))
...
if(!(file.exists(output_topology_file))){
  ...
  # Calculate topological parameters depending on the level of correlation of the edges
  type_correlation_df = data.frame(row.names=c("all", "very weak", "weak", "moderate", "strong", "very strong"), lower_val=c(0,0,0.2,0.4,0.6,0.8), upper_val=c(1,0.2,0.4,0.6,0.8,1))
  for(type_correlation in row.names(type_correlation_df)){
    
    if(type_correlation %in% c("all", "very strong")){
      coexpression_filt_df = coexpression_df %>% filter(score >= type_correlation_df[row.names(type_correlation_df) == type_correlation,]$lower_val)
    }else{
      coexpression_filt_df = coexpression_df %>% filter((score >= type_correlation_df[row.names(type_correlation_df) == type_correlation,]$lower_val) & (score < type_correlation_df[row.names(type_correlation_df) == type_correlation,]$upper_val))    
    }
    coexpression_filt_net = graph_from_data_frame((coexpression_filt_df %>% rename("weight"="score")), directed=F) %>% simplify()
    print(paste("Calculating topology parameters for correlation: ", type_correlation, sep=""))
    
    # Calculate topological parameters
    num_nodes = gorder(coexpression_filt_net)
    num_edges = gsize(coexpression_filt_net)
    
    start_time <- Sys.time()
    print("Calculating average degree")
    av_degree = mean(degree(coexpression_filt_net))
    end_time <- Sys.time()
    time_diff = end_time - start_time
    print(time_diff)
    ...
    av_path_length = mean_distance(coexpression_filt_net)
    end_time <- Sys.time()
    time_diff = end_time - start_time
    print(time_diff)
    ...
    av_clustering_coef = transitivity(coexpression_filt_net)
    end_time <- Sys.time()
    time_diff = end_time - start_time
    print(time_diff)
    ...
    components_net = components(coexpression_filt_net)
    lcc = induced.subgraph(coexpression_filt_net, vids = V(coexpression_filt_net)[components_net$membership == which.max(components_net$csize)] )
    ...
    k_core = coreness(coexpression_filt_net)
    max_k = max(k_core)
    main_core = induced.subgraph(coexpression_filt_net, vids = names(k_core[k_core==max_k]))
...
if(!(file.exists(output_ppi_file))){
  ...
  # Calculate PPI parameters depending on the level of correlation of the edges
  type_correlation_df = data.frame(row.names=c("all", "very weak", "weak", "moderate", "strong", "very strong"), lower_val=c(0,0,0.2,0.4,0.6,0.8), upper_val=c(1,0.2,0.4,0.6,0.8,1))
  for(type_correlation in row.names(type_correlation_df)){
    ...
    coexpression_filt_net = graph_from_data_frame((coexpression_filt_df %>% rename("weight"="score")), directed=F) %>% simplify()
    print(paste("Calculating PPI parameters for correlation: ", type_correlation, sep=""))
    
    # Select PPIs in co-expression network
    coexpression_ppi_df = inner_join(coexpression_filt_df, ppi_df, by=c("Node.1"="proteinA_symbol", "Node.2"="proteinB_symbol"))
...
if(!(file.exists(output_disease_genes_file))){
  ...
  # Calculate disease gene parameters depending on the level of correlation of the edges
  type_correlation_df = data.frame(row.names=c("all", "very weak", "weak", "moderate", "strong", "very strong"), lower_val=c(0,0,0.2,0.4,0.6,0.8), upper_val=c(1,0.2,0.4,0.6,0.8,1))
  for(type_correlation in row.names(type_correlation_df)){
    ...
    coexpression_filt_net = graph_from_data_frame((coexpression_filt_df %>% rename("weight"="score")), directed=F) %>% simplify()
    print(paste("Calculating disease gene parameters for correlation: ", type_correlation, sep=""))
    ...
    for (disease in selected_diseases){
      ...
      disease_selected_genes_in_network = disease_selected_genes[disease_selected_genes %in% V(coexpression_filt_net)$name]
      disease_subgraph = induced.subgraph(coexpression_filt_net, vids=disease_selected_genes_in_network)
      disease_net_df = coexpression_filt_df %>% filter((Node.1 %in% V(disease_subgraph)$name) & (Node.2 %in% V(disease_subgraph)$name))
      ...
      #disease_lcc_sig = LCC_Significance(N = 1000, Targets = V(disease_lcc)$name, G = coexpression_filt_net, bins =1) # With bin=1, the degree distribution is not maintained and the hypothesis changes
...
if(!(file.exists(output_essential_genes_file))){
  ...
  # Calculate essential gene parameters depending on the level of correlation of the edges
  type_correlation_df = data.frame(row.names=c("all", "very weak", "weak", "moderate", "strong", "very strong"), lower_val=c(0,0,0.2,0.4,0.6,0.8), upper_val=c(1,0.2,0.4,0.6,0.8,1))
  for(type_correlation in row.names(type_correlation_df)){
    ...
    coexpression_filt_net = graph_from_data_frame((coexpression_filt_df %>% rename("weight"="score")), directed=F) %>% simplify()
    print(paste("Calculating essential gene parameters for correlation: ", type_correlation, sep=""))
    
    # Essential genes analysis
    essential_genes_in_network = essential_genes[essential_genes %in% V(coexpression_filt_net)$name]
    essential_subgraph = induced.subgraph(coexpression_filt_net, vids=essential_genes_in_network)
    essential_subgraph_df = coexpression_filt_df %>% filter((Node.1 %in% V(essential_subgraph)$name) & (Node.2 %in% V(essential_subgraph)$name))
    ...
    #essential_lcc_sig = LCC_Significance(N = 1000, Targets = V(essential_lcc)$name, G = coexpression_filt_net)
