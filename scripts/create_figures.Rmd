...
predicted_results_mean_small_example_df$model <- factor(predicted_results_mean_small_example_df$model, levels = c("Mean", "Stretched exp.", "Logarithm", "Square root", "Linear", "Exponential"))
...
(((curve_plot +
  ...
  ((comparison_plot +
    ...
    (prediction_plot +
     ...
     labs(tag = 'G') +
     theme(legend.position="none"))) /
   ((wrap_elements(figure_summary_table_plot) + labs(tag='E')) | 
...
diseases_selected = c("arthritis rheumatoid", "asthma", "breast neoplasms", "thyroid neoplasms")
...
for (x in 1:length(datasets_selected)){
  ...
  mutate(mean_pval = replace(mean_pval, mean_pval < 0.000001, 0.000001)) %>%
    ...
    #scale_x_continuous(trans = scales::log10_trans()) +
    scale_color_manual(values = c("#65F3BF", "#d9f365", "#0072B2", "#56B4E9")) +
    theme_linedraw() +
    xlab("Number of samples") +
    #xlab("log(N)") +
  ...
  module_signif_by_disease_plots[[x]] = module_signif_by_disease_plot
  module_signif_by_disease_plot = module_signif_by_disease_plot + 
      guides(col=guide_legend(title="Disease")) + #Add legend separately
  print(module_signif_by_disease_plot)
  plot_file = paste(plots_dir, "/disease_module_significance_by_diseases_pearson_pval_0.05_data_", dataset_selected, ".png", sep="")
  ggsave(
    plot_file,
...
```{r}
module_signif_by_correlation_plot = results_scipher_df %>%
...
module_signif_by_correlation_plot_to_save = module_signif_by_correlation_plot + 
  guides(col=guide_legend(title="Correlation level"))
print(module_signif_by_correlation_plot_to_save)
plot_file = paste(plots_dir, "disease_module_significance_by_correlations_pearson_pval_0.05_data_sciphercomplete.png", sep="/")
ggsave(
  plot_file,
  plot = module_signif_by_correlation_plot_to_save,
  dpi = 1200,
  #width = 9000,
  #height = 6000,
  units = c("px")
...
```{r}
module_rlcc_by_correlation_plot = results_scipher_df %>%
  filter(disease == "arthritis rheumatoid") %>%
  filter((type_dataset == dataset_selected) & (threshold == threshold_selected) & (type_correlation %in% c("weak-moderate-strong-very strong", "moderate-strong-very strong", "strong-very strong", "very strong"))) %>%
  mutate(type_correlation = replace(type_correlation, type_correlation == "weak-moderate-strong-very strong", "\u2265 0.2")) %>%
  mutate(type_correlation = replace(type_correlation, type_correlation == "moderate-strong-very strong", "\u2265 0.4")) %>%
...
module_rlcc_by_correlation_plot_to_save = module_rlcc_by_correlation_plot + 
  guides(col=guide_legend(title="Correlation level"))
print(module_rlcc_by_correlation_plot_to_save)
plot_file = paste(plots_dir, "disease_module_rlcc_by_correlations_pearson_pval_0.05_data_sciphercomplete.png", sep="/")
ggsave(
  plot_file,
  plot = module_rlcc_by_correlation_plot_to_save,
  dpi = 1200,
  #width = 9300,
  #height = 6000,
  units = c("px")
...
module_signif_by_dataset_plot = all_disease_genes_results_df %>%
  ...
  mutate(type_dataset = replace(type_dataset, type_dataset == "gtex:thyroid", "GTEx: Thyroid")) %>%
  mutate(type_dataset = replace(type_dataset, type_dataset == "gtex:breast.mammary.tissue", "GTEx: Breast")) %>%
  mutate(type_dataset = replace(type_dataset, type_dataset == "tcga:tcga-thca", "TCGA: Thyroid c.")) %>%
  mutate(type_dataset = replace(type_dataset, type_dataset == "tcga:tcga-brca", "TCGA: Breast c.")) %>%
...
module_signif_by_dataset_plot_to_save = module_signif_by_dataset_plot + 
  guides(col=guide_legend(title="Dataset"))
print(module_signif_by_dataset_plot_to_save)
plot_file = paste(plots_dir, "disease_module_significance_by_dataset_pearson_pval_0.05.png", sep="/")
ggsave(
  plot_file,
  plot = module_signif_by_dataset_plot_to_save,
  dpi = 1200,
  #width = 9200,
  #height = 6000,
  units = c("px")
...
```{r}
# Use patchwork to concatenate plots
(((module_rlcc_by_correlation_plot | module_signif_by_correlation_plot) + 
    plot_layout(guides = 'collect') &
    guides(col=guide_legend(title="Correlation level", title.position="top", nrow=1,byrow=TRUE)) & # Place legend in two columns
   ...
   ((module_signif_by_disease_plots[[6]] + 
       guides(col=guide_legend(title="Disease", title.position="top", nrow=4,byrow=TRUE)) +
       theme(legend.position = 'bottom', legend.text = element_text(size = 11),  legend.title=element_text(size=13, face="bold"))) | 
    (module_signif_by_dataset_plot + 
       guides(col=guide_legend(title="Dataset", title.position="top", nrow=3,byrow=TRUE)) +
       theme(legend.position = 'bottom', legend.text = element_text(size = 11),  legend.title=element_text(size=13, face="bold"))))) + 
...
plot_file = paste(plots_dir, "/disease_module_results.png", sep="")
ggsave(
  plot_file,
  dpi = 1200,
  width = 9000,
