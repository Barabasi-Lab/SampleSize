...
Now, filter by the disease selected:
...
### Analyze the nodes of breast carcinoma

First compile all files and order them for the comparisons:
...
Then analyze files, first counting genes per category and then comparing networks of different sizes:
...
#cols = c("Node", "DiseaseName.no.sp.char", "Phi_tilde_prev", "size_prev", "rep_D_prev", "rep_N_prev", "Phi_tilde_curr", "size_curr", "rep_D_curr", "rep_N_curr")
#change_nodes_df = data.frame(matrix(ncol=length(cols),nrow=0, dimnames=list(NULL, cols)))
...
for(x in seq(sizes_list)){
  ...
  for(input_file in input_files){
    ...
    # if(nrow(same_size_info_df) == 0){
    #   same_size_info_df = cbind(results_count$node_results_filt_df, data.frame(size=size, rep_D=r_D, rep_N=r_N))
    # } else {
    #   same_size_info_df = rbind(same_size_info_df, cbind(results_count$node_results_filt_df, data.frame(size=size, rep_D=r_D, rep_N=r_N)))
    # }
  }
  # if(x > 1){
  #   reps_curr = unique(paste(same_size_info_df$rep_D, same_size_info_df$rep_N, sep="-"))
  #   reps_prev = unique(paste(previous_size_info_df$rep_D, previous_size_info_df$rep_N, sep="-"))
  #   for(rep_curr in reps_curr){
  #     curr_df = same_size_info_df %>% filter((rep_D == strsplit(rep_curr, split="-")[[1]][1]) & (rep_N == strsplit(rep_curr, split="-")[[1]][2]))
  #     for(rep_prev in reps_prev){
  #       print(c(rep_curr, rep_prev))
  #       prev_df = previous_size_info_df %>% filter((rep_D == strsplit(rep_prev, split="-")[[1]][1]) & (rep_N == strsplit(rep_prev, split="-")[[1]][2]))
  #       change_df = (prev_df %>% select("Node", "DiseaseName.no.sp.char", "Phi_tilde", "size", "rep_D", "rep_N") %>% rename("Phi_tilde_prev"="Phi_tilde", "size_prev"="size", "rep_D_prev"="rep_D", "rep_N_prev"="rep_N")) %>% full_join((curr_df %>% select("Node", "DiseaseName.no.sp.char", "Phi_tilde", "size", "rep_D", "rep_N") %>% rename("Phi_tilde_curr"="Phi_tilde", "size_curr"="size", "rep_D_curr"="rep_D", "rep_N_curr"="rep_N")), by=c("Node", "DiseaseName.no.sp.char"))
  #       change_nodes_df = rbind(change_nodes_df, change_df)
  #     }
  #   }
  # }
  # previous_size_info_df = data.frame(same_size_info_df)
}

head(counts_nodes_df)
head(nodes_df)
#head(change_nodes_df)
```
...
### Analyze the nodes of rheumatoid arthritis
...
Compile all files and order them for the comparisons:
