...
networks_dir = '/scratch/j.aguirreplans/Scipher/SampleSize/networks_GTEx_adj'
    ...
    if (length(tissue_sex_samples) > 10){
      ...
      for (size in size_list){
        ...
        for(r in 1:rep){
          ...
          tissue_sex_network_df = fread(tissue_sex_network_file)
          cutoffs_list = seq(0.1, 0.9, by = 0.1)
          for (cutoff in cutoffs_list){
            print(cutoff)
            tissue_sex_network_filt_df = tissue_sex_network_df[tissue_sex_network_df$WGCNA > cutoff,] %>% select(Node.1, Node.2)
