...
However, there is still a small number of samples that might belong to the same participant and also come from the same type of tissue. In these cases, the sample information might come from different vials, portions or plates.
See this website for more information: https://docs.gdc.cancer.gov/Encyclopedia/pages/TCGA_Barcode/  

```{r}
head(metadata_df %>% select(file_id, entity_submitter_id, participant, sample_code, portion_code, plate, center, sample_type, project_id) %>% unique() %>% group_by(participant) %>% filter(n()>1) %>% mutate(num_sample_types_per_participant=n_distinct(sample_type)) %>% mutate(num_samples_per_participant=n_distinct(file_id)) %>% filter((num_sample_types_per_participant == 1 & num_samples_per_participant > 1)) %>% ungroup(), 12)
```

This means that later, we will have to merge the expression of the samples that are from same participant and come from the same tissue type, but are from different vials, portions or plates. We will do it by using the participant_sample code:

```{r}
if(!("participant_sample" %in% colnames(metadata_df))){
  metadata_df$participant_sample = paste(metadata_df$participant, metadata_df$sample, sep="-")
...
tcga_file = paste(output_dir, 'TCGA_tpm.csv', sep='/')
...
```{r}
tcga_processed_file = paste(output_dir, 'TCGA_tpm_genes_processed.csv', sep='/')
...
There are participant_sample IDs that are mapped to multiple files. We will merge the participant_sample IDs that have multiple file IDs, and substitute the file IDs with participant_sample IDs in the data file.

We identify the participant_sample IDs with multiple file IDs:

```{r}
mappings_tcga_df = metadata_df %>% filter(file_name %in% colnames(tcga_df)[-1]) %>% select(file_name, participant_sample) %>% unique()
mappings_tcga_duplicated_df = mappings_tcga_df %>% filter(participant_sample %in% subset(mappings_tcga_df,duplicated(participant_sample))$participant_sample)
head(mappings_tcga_duplicated_df)
```

There are `r length(unique(mappings_tcga_duplicated_df$participant_sample))` participant_sample IDs with multiple file IDs. We calculate the median between the expression of these file IDs.

```{r aggregate_participant_sample_ids}
...
tcga_df %<>% relocate(file_name, participant_sample)
tcga_df = subset(tcga_df, select=-file_name)
tcga_participant_ids_duplicated = tcga_df[tcga_df$participant_sample %in% unique(mappings_tcga_duplicated_df$participant_sample),]
#participant_ids_aggregated = aggregate(.~participant_sample, tcga_participant_ids_duplicated, median)
participant_ids_aggregated = tcga_participant_ids_duplicated %>% group_by(participant_sample) %>% summarise(across(everything(), median, na.rm = TRUE)) %>% ungroup() %>% as.data.frame()
tcga_df = rbind(tcga_df[!(tcga_df$participant_sample %in% unique(mappings_tcga_duplicated_df$participant_sample)),], participant_ids_aggregated)
dim(tcga_df)
```

```{r transpose_processed_matrix}
# We transpose the matrix
genes = colnames((tcga_df %>% select(-participant_sample)))
participant_sample_ids = tcga_df$participant_sample
tcga_df = t(as.matrix((tcga_df %>% select(-participant_sample)))) %>% as.data.frame()
colnames(tcga_df) = participant_sample_ids
...
The processed `tcga_df` dataframe contains `r as.integer(ncol(tcga_df)-1)` columns (participant_sample IDs) and `r nrow(tcga_df)` rows (genes).
