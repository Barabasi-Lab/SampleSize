...
def parse_options():
    ...
    if options.input_dir is None or options.output_dir is None or options.metric is None:
...
def create_gene_coexpression_networks(options):
    ...
    script_file = os.path.join(src_path, 'create_gene_coexpression_network.R')
    ...
    for dataset in sorted(datasets):
        ...
        if not fileExist(output_file):
            ...
            command = 'Rscript {} -f {} -o {} -m {}'.format(script_file, input_file, output_file, metric)
            print(command)
