...
genes = colnames(RNA)
sample_ids = RNA_metadata$Scipher_id.visit.batch
...
* Standard deviation of all samples above 0: `r length(info_df[info_df$sd > 0,]$Gene)`.
...
```{r}
RNAseq_processed = RNA_metadata %>% select(c("Scipher_id.visit.batch", "Scipher_id", "visit", "batch", info_filt_df$Gene))
RNAseq_processed_t = t(as.matrix(RNAseq_processed %>% select(info_filt_df$Gene))) %>% as.data.frame()
colnames(RNAseq_processed_t) = RNA_metadata$Scipher_id.visit.batch
...
We end up having `r length(RNAseq_processed_t$Gene)` genes and `r length(unique(RNAseq_processed$Scipher_id.visit.batch))` samples from `r length(unique(RNAseq_processed$Scipher_id))` individuals.
