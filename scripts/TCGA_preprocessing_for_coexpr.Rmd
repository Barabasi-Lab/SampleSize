...
tcga_processed_file = paste(output_dir, 'TCGA_tpm_genes_samples_processed.csv', sep='/')
...
for(sample_type_selected in unique(metadata_df$sample_type_simplified)){
  ...
  # We write final file
  tcga_final_file = paste(filter_zeros_dir, 'TCGA_tpm_processed_filter_genes_with_zeros.csv', sep='/')
  ...
  genes_dataset_file = paste(filter_zeros_dir, 'rnaseq_gene_info_filter_genes_with_zeros.csv', sep='/')
...
info_filt_df = info_df[(info_df$sum > 0) & (info_df$mean > 1) & (info_df$median > 1) & (info_df$sd > 0) & (info_df$sum_zero_counts < dim(RNAseq_by_type[, -1])[2]/2) ,]
  ...
  # We write final file
  tcga_final_file = paste(filter_low_counts_dir, 'tcga_tpm_processed_filter_genes_low_counts.csv', sep='/')
  ...
  # Write the information about the selected genes of the dataset in a separate file
  genes_dataset_df$enough_counts = ifelse(genes_dataset_df$HGNC_Symbol %in% info_filt_df$Gene, TRUE, FALSE)
  genes_dataset_file = paste(filter_low_counts_dir, 'rnaseq_gene_info_filter_genes_low_counts.csv', sep='/')
  fwrite(genes_dataset_df, genes_dataset_file)
  # Check the percentage of genes in each category after filtering by counts
  genes_dataset_filtered_df = genes_dataset_df %>% filter(HGNC_Symbol %in% info_filt_df$Gene) %>% unique
  table(genes_dataset_filtered_df$HGNC_category)
...
for(sample_type_selected in unique(metadata_df$sample_type_simplified)){
  ...
  for (project_id_selected in projects){
    ...
    if(length(samples_project) >= 10){
      ...
      info_filt_df = info_df[(info_df$sum > 0) & (info_df$mean > 5) & (info_df$median > 5) & (info_df$sd > 1) & (info_df$sum_zero_counts < dim(RNAseq_project[, -1])[2]/2) ,]
      ...
      # Count statistics about the filter
      statistics_low_counts_df = rbind(statistics_low_counts_df, data.frame(project=project_id_selected, n_genes_raw=length(RNAseq_project$Gene), n_genes_processed=length(RNAseq_project_filt$Gene), sum=length(info_df[(info_df$sum > 0),]$Gene), mean=length(info_df[(info_df$mean > 1),]$Gene), median=length(info_df[(info_df$median > 1),]$Gene), sd=length(info_df[(info_df$sd > 0),]$Gene), sum_zero_counts=length(info_df[(info_df$sum_zero_counts < dim(RNAseq_project[, -1])[2]/2),]$Gene)))
