...
topology_results_df$dataset = ifelse(topology_results_df$type_tcga_tissue == "normal", paste(topology_results_df$dataset, "normal", sep="-"), topology_results_df$dataset)
...
```{r}
topology_results_selected_df = topology_results_df %>% filter((method == "pearson") & (dataset == "tcga:tcga-brca_female") & (threshold == 0.05) & (type_correlation %in% c("all", "very strong", "strong-very strong", "moderate-strong-very strong", "weak-moderate-strong-very strong"))) %>% select(size, rep, num_edges, type_correlation)
topology_results_selected_by_size_df = topology_results_selected_df %>%
