...
numbers_df <- rbind(numbers_df, data.frame(dataset="scipher:complete.dataset", total_num_nodes=num_genes, total_num_edges=num_edges))
...
#genes_project_files = list.files(genes_filt_dir)
projects = c("TCGA-BRCA", "TCGA-UCEC")
for (project_id in projects){
  ...
  num_genes = length(unique((genes_dataset_df %>% filter(enough_counts == TRUE) %>% unique())$HGNC_Symbol)) # Total number of genes
  num_edges = num_genes*(num_genes-1)/2 # Total number of edges
  numbers_df <- rbind(numbers_df, data.frame(dataset=paste("tcga", project_id, sep=":"), total_num_nodes=num_genes, total_num_edges=num_edges))
}
